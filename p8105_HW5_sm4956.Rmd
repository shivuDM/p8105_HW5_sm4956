...
Repeating the simulation with other values of *μ= {1,2,3,4,5,6}* and generating 5000 datasets for these, along with for μ=0. For each dataset, saving μ^ and the p-value arising from test of H:μ=0 using α=0.05.
```{r repeating the simulations with others values of mu and generating 5000 datasets for all values of mu}
sim_all_mu = tibble(
...
Adding a variable to see if the null was rejected (TRUE) or not (FALSE) to the previous dataset sim_all_mu
```{r variable for rejection of null}
...
Making plot to show the proportion of times the null was rejected (the power of the test) on the y axis and the true value of μ on the x axis.
```{r plotting for power and mu}
...
It can be seen that as the true value of μ increases, the power of the test (proportion of times the null was rejected) also increases. Therefore, we can say that as the effect size increases, i.e., true value of μ goes away from null=0, the power of the test to reject the null also increases.

Creating separate samples to see the average of when the null was rejected, and for the total average for all the values of μ. Making a final dataset avg_estimates to merge these separate datasets and view the average estimates of both the samples.
```{r average}
...
Creating plots showing the average estimate of μ^ on the y axis and the true value of μ on the x axis, and overlay showing the average estimate of μ^ only in samples for which the null was rejected on the y axis and the true value of μ on the x axis.
```{r plot for average estimate of μ^ and true value of μ}
all_avg_plot = all_avg %>%
  ggplot (aes(x= mu_value, y= mean_all)) +
  geom_point (alpha = 0.5) + 
  geom_line () +
labs (title = "Plot for true value of μ vs Average estimate of μ^ for all estimates",
        x = "True value of μ",
        y = "Average estimate of μ^")
all_avg_plot
```

```{r plot for average estimate of μ^ and true value of μ when Null rejected}
rejected_avg_plot = rejected_avg %>%
  ggplot (aes(x= mu_value, y= mean_rejected)) +
  geom_point (alpha = 0.5) + 
  geom_line () +
labs (title = "Plot for true value of μ vs Average estimate of μ^ when null was rejected",
        x = "True value of μ",
        y = "Average estimate of μ^ for Null-rejected")
rejected_avg_plot
```

```{r overlay plot for average estimate of μ^ and true value of μ when Null rejected}
...
labs (title = "Overlay Plot for true value of μ vs Average estimate of μ^ in sample for Null-rejected",
        x = "True value of μ",
        y = "Average estimate of μ^ for Null-rejected")
overlay_avg_plot
```

The overlay plot shows that the true mean and average estimate of mean are not same/ diverge when the mean/average value is small (i.e., for values 1,2,3). They converge or are approximately equal when the mean gets larger (i.e., for values 4,5,6). 
The reason is that when the true mean is greater or away from the null, higher proportion of rejection due to larger effect size. So, when the sample average of μ^ across tests for which the null is rejected is larger, it converges with the effect size/ true value of μ in terms of power to reject the null.